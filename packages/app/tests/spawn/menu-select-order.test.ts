...
import { buildSelectLabels, buildSelectListWindow } from "../../src/spawn/menu-render-select.js"
import { sortItemsByLaunchTime } from "../../src/spawn/menu-select-order.js"
import type { SelectProjectRuntime } from "../../src/spawn/menu-types.js"
...
describe("menu-select order", () => {
  it("sorts projects by last container start time (newest first)", () => {
    const newest = makeProjectItem({ projectDir: "/home/dev/.spawn/newest", displayName: "org/newest" })
    const older = makeProjectItem({ projectDir: "/home/dev/.spawn/older", displayName: "org/older" })
    const neverStarted = makeProjectItem({ projectDir: "/home/dev/.spawn/never", displayName: "org/never" })
  ...
  it("shows container launch timestamp in select labels", () => {
    const item = makeProjectItem({ projectDir: "/home/dev/.spawn/example", displayName: "org/example" })
