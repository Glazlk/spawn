...
import { defaultTemplateConfig } from "@spawn-dev/lib/core/domain"
import { renderEntrypoint } from "@spawn-dev/lib/core/templates-entrypoint"
...
describe("renderEntrypoint auth bridge", () => {
  it.effect("maps GH token fallback to git auth and sets git credential helper", () =>
    Effect.sync(() => {
      ...
      expect(entrypoint).toContain("spawn_upsert_ssh_env \"GITHUB_TOKEN\" \"$EFFECTIVE_GITHUB_TOKEN\"")
      expect(entrypoint).toContain("spawn_upsert_ssh_env \"GH_TOKEN\" \"$EFFECTIVE_GH_TOKEN\"")
      expect(entrypoint).toContain("spawn_upsert_ssh_env \"GIT_AUTH_TOKEN\" \"$EFFECTIVE_GITHUB_TOKEN\"")
      expect(entrypoint).toContain("GIT_CREDENTIAL_HELPER_PATH=\"/usr/local/bin/spawn-credential-helper\"")
      expect(entrypoint).toContain("CLAUDE_REAL_DIR=\"$(dirname \"$CURRENT_CLAUDE_BIN\")\"")
      expect(entrypoint).toContain("CLAUDE_REAL_BIN=\"$CLAUDE_REAL_DIR/.spawn-claude-real\"")
      ...
      expect(entrypoint).toContain("spawn_ensure_claude_cli()")
      ...
      expect(entrypoint).toContain("spawn_sync_claude_permissions()")
      expect(entrypoint).toContain(
      ...
      expect(entrypoint).toContain("spawn_prepare_claude_auth_mode()")
      expect(entrypoint).toContain(
      ...
      expect(entrypoint).toContain("command: \"spawn-playwright-mcp\"")
      ...
      expect(entrypoint).toContain("spawn_link_claude_home_file()")
      expect(entrypoint).toContain("spawn_link_claude_home_file \".oauth-token\"")
      expect(entrypoint).toContain("spawn_link_claude_home_file \".config.json\"")
      expect(entrypoint).toContain("spawn_link_claude_home_file \".claude.json\"")
      expect(entrypoint).toContain("spawn_link_claude_home_file \".credentials.json\"")
      expect(entrypoint).toContain(
        "spawn_link_claude_file \"$CLAUDE_CONFIG_DIR/.claude.json\" \"$CLAUDE_HOME_JSON\""
      )
      ...
      expect(entrypoint).toContain("spawn-managed:claude-md")
      expect(entrypoint).toContain(
