...
import { resolveMenuStartupSnapshot } from "../../src/spawn/menu-startup.js"
import { makeProjectItem } from "./fixtures/project-item.js"

describe("menu-startup", () => {
  it("returns empty snapshot when no spawn containers are running", () => {
  ...
  it("auto-selects active project when exactly one known spawn container is running", () => {
  ...
  it("does not auto-select when multiple spawn containers are running", () => {
    const first = makeProjectItem({
      containerName: "dg-one",
      displayName: "org/one",
      projectDir: "/home/dev/.spawn/org-one"
    })
    const second = makeProjectItem({
      containerName: "dg-two",
      displayName: "org/two",
      projectDir: "/home/dev/.spawn/org-two"
    })
  ...
  it("shows warning when running spawn containers have no matching configs", () => {
