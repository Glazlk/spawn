...
const renderRunningHint = (runningCount: number): string =>
  runningCount === 1
    ? "Detected 1 running spawn container."
    : `Detected ${runningCount} running spawn containers.`

// CHANGE: infer initial menu state from currently running spawn containers
// WHY: avoid "(none)" confusion when containers are already up outside this TUI session
// QUOTE(ISSUE): "У меня запущены контейнеры от spawn но он говорит что они не запущены"
// REF: issue-13
// SOURCE: n/a
