...
import type { AttachCommand, PanesCommand } from "@spawn-dev/lib/core/domain"
import { deriveRepoPathParts, deriveRepoSlug } from "@spawn-dev/lib/core/domain"
import {
  runCommandCapture,
  runCommandExitCode,
  runCommandWithExitCodes
} from "@spawn-dev/lib/shell/command-runner"
import { readProjectConfig } from "@spawn-dev/lib/shell/config"
import type {
  ConfigDecodeError,
  ConfigNotFoundError,
  DockerCommandError,
  FileExistsError,
  PortProbeError
} from "@spawn-dev/lib/shell/errors"
import { CommandFailedError } from "@spawn-dev/lib/shell/errors"
import { resolveBaseDir } from "@spawn-dev/lib/shell/paths"
import { findSshPrivateKey } from "@spawn-dev/lib/usecases/path-helpers"
import { buildSshCommand } from "@spawn-dev/lib/usecases/projects"
import { runDockerComposeUpWithPortCheck } from "@spawn-dev/lib/usecases/projects-up"
...
const readLayoutVersion = (
  ...
  runTmuxCapture(["show-options", "-t", session, "-v", "@spawn-layout"]).pipe(
...
const configureSession = (
  ...
  Effect.gen(function*(_) {
    yield* _(runTmux(["set-option", "-t", session, "@spawn-layout", layoutVersion]))
    ...
    yield* _(runTmux(["set-option", "-t", session, "status-left", ` spawn :: ${repoDisplayName} `]))
...
// CHANGE: list tmux panes for a spawn project
...
> =>
  Effect.gen(function*(_) {
    ...
    const session = `spawn-${deriveRepoSlug(config.template.repoUrl)}`
    const hasSessionCode = yield* _(runTmuxExitCode(["has-session", "-t", session]))
    if (hasSessionCode !== 0) {
      yield* _(Effect.logWarning(`tmux session ${session} not found. Run 'spawn attach' first.`))
      return
    }
    const raw = yield* _(
      runTmuxCapture([
...
// CHANGE: attach a tmux workspace for a spawn project
...
> =>
  Effect.gen(function*(_) {
    ...
    const session = `spawn-${deriveRepoSlug(template.repoUrl)}`
    const hasSessionCode = yield* _(runTmuxExitCode(["has-session", "-t", session]))
