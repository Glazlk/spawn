...
import { buildCreateCommand, nonEmpty } from "@spawn-dev/lib/core/command-builders"
import type { RawOptions } from "@spawn-dev/lib/core/command-options"
import { type Command, type ParseError, resolveRepoInput } from "@spawn-dev/lib/core/domain"
...
const applyCloneDefaults = (
  ...
  return {
    ...raw,
    repoUrl: rawRepoUrl,
    outDir: raw.outDir ?? `.spawn/${repoPath}`,
    targetDir: raw.targetDir ?? `${targetHome}/workspaces/${repoPath}`
  }
}

// CHANGE: parse clone command with positional repo url
// WHY: allow "spawn clone <url>" to build + run a container
// QUOTE(ТЗ): "spawn clone url"
