...
import type { ParseError } from "@spawn-dev/lib/core/domain"

export const usageText = `spawn menu
spawn create [--repo-url <url>] [options]
spawn clone <url> [options]
spawn open [<url>] [options]
spawn apply [<url>] [options]
spawn mcp-playwright [<url>] [options]
spawn attach [<url>] [options]
spawn panes [<url>] [options]
spawn scrap <action> [<url>] [options]
spawn sessions [list] [<url>] [options]
spawn sessions kill <pid> [<url>] [options]
spawn sessions logs <pid> [<url>] [options]
spawn ps
spawn down-all
spawn auth <provider> <action> [options]
spawn state <action> [options]

Commands:
  menu                Interactive menu (default when no args)
  create, init        Generate docker development environment (repo URL optional)
  clone               Create + run container and clone repo
  open                Open existing spawn project workspace
  apply               Apply spawn config to an existing project/container (current dir by default)
  mcp-playwright      Enable Playwright MCP + Chromium sidecar for an existing project dir
  attach, tmux        Alias for open
  panes, terms        List tmux panes for a spawn project
  scrap               Export/import project scrap (session snapshot + rebuildable deps)
  sessions            List/kill/log container terminal processes
  ps, status          Show docker compose status for all spawn projects
  down-all            Stop all spawn containers (docker compose down)
  auth                Manage GitHub/Codex/Claude Code auth for spawn
  state               Manage spawn state directory via git (sync across machines)
...
Options:
  ...
  --shared-network <name>   Shared Docker network name when network-mode=shared (default: spawn-shared)
...
Container runtime env (set via .orch/env/project.env):
  ...
  CLAUDE_AUTO_SYSTEM_PROMPT=1|0         Auto-attach spawn managed system prompt to claude (default: 1)
  SPAWN_ZSH_AUTOSUGGEST=1|0        Enable zsh-autosuggestions (default: 1)
  SPAWN_ZSH_AUTOSUGGEST_STYLE=...  zsh-autosuggestions highlight style (default: fg=8,italic)
  SPAWN_ZSH_AUTOSUGGEST_STRATEGY=...  Suggestion sources (default: history completion)
...
State actions:
  state path                         Print current projects root (default: ~/.spawn; override via SPAWN_PROJECTS_ROOT)
