...
export const selectTitle = (purpose: SelectPurpose): string =>
  Match.value(purpose).pipe(
    Match.when("Connect", () => "spawn / Select project"),
    Match.when("Auth", () => "spawn / Project auth"),
    Match.when("Down", () => "spawn / Stop container"),
    Match.when("Info", () => "spawn / Show connection info"),
    Match.when("Delete", () => "spawn / Delete project"),
    Match.exhaustive
  )
